    ...
    public void onReaderEvent(ReaderEvent readerEvent)
    {
        if(readerEvent.getEventType().equals(com.tikitag.client.tagservice.ReaderEvent.ReaderEventType.READER_ADDED))
        {
            ...
            for(int i = 1; i <= prefs.getInt("readers.index", 0); i++)
                if(readerSn.equals(prefs.get((new StringBuilder()).append("readers.list.snreader.").append(i).toString(), null)))
                    readerFound = true;
            ...
            if(!readerFound)
            {
                ...
                prefs.put((new StringBuilder()).append("readers.list.snreader.").append(newReaderIndex).toString(), readerSn);
                NewReaderEvent newReaderEvent = new NewReaderEvent(readerEvent.getReaderId());
            } else
            {
